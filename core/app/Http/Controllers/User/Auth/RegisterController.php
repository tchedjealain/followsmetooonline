...
class RegisterController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if (preg_match("/[^a-z0-9_]/",
            ...
            $notify[] = ['info', 'Le nom d\'utilisateur ne peut contenir que des lettres minuscules, chiffres et un underscore'];
            $notify[] = ['error', 'Aucun caractère spécial, espace ou majuscule dans le nom d\'utilisateur.'];
            return back()->withNotify($notify)->withInput($request->all());
        }

        if (!verifyCaptcha()) {
            $notify[] = ['error', 'Captcha fourni invalide'];
        ...
        if ($exist) {
            $notify[] = ['error', 'Le numéro de téléphone existe déjà'];
    ...
    protected function create(array $data)
    {
        ...
        $adminNotification->title = 'Nouveau membre inscrit';
